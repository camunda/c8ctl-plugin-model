...
test('append signal-intermediate-throw-event with --signal-name creates signal ref', async () => {
    ...
    try {
        ...
        const evDef = el['eventDefinition'];
...
test('update signalRef throws on element without signal event definition', async () => {
    ...
    try {
        ...
        await assert.rejects(() => update(['signalRef', 'someSignal'], cwd), /does not have a signal event definition/);
    }
    finally {
        cleanup(cwd);
    }
...
test('update messageRef throws on element without message event definition', async () => {
    ...
    try {
        ...
        await assert.rejects(() => update(['messageRef', 'someMessage'], cwd), /does not have a message event definition/);
    }
    finally {
        cleanup(cwd);
    }
...
test('cursor-status includes eventDefinition for signal event with ref', async () => {
    ...
    try {
        ...
        const logs = [];
        const logger = { json: (obj) => { logs.push(obj); } };
        await cursorStatus([], cwd, logger);
        assert.equal(logs.length, 1);
        const result = logs[0];
        const evDef = result['eventDefinition'];
        assert.equal(evDef['type'], 'signal');
...
test('end-to-end: signal throw and catch share same signalRef', async () => {
    ...
    try {
        ...
        const throwDef = throwEl?.['eventDefinition'];
        const catchDef = catchEl?.['eventDefinition'];
...
test('--signal-name on message event throws clear error', async () => {
    ...
    try {
        await setupModel('proc', cwd);
        await assert.rejects(() => append(['message-intermediate-catch-event', 'Wait', '--signal-name', 'badSignal'], cwd), /--signal-name can only be used with signal events/);
    }
    finally {
        cleanup(cwd);
    }
...
test('--message-name on signal event throws clear error', async () => {
    ...
    try {
        await setupModel('proc', cwd);
        await assert.rejects(() => append(['signal-intermediate-throw-event', 'Throw', '--message-name', 'badMessage'], cwd), /--message-name can only be used with message events/);
    }
    finally {
        cleanup(cwd);
    }
...
test('--signal-name on timer event throws clear error', async () => {
    ...
    try {
        await setupModel('proc', cwd);
        await assert.rejects(() => append(['timer-intermediate-catch-event', 'Wait', '--signal-name', 'badSignal'], cwd), /--signal-name can only be used with signal events/);
    }
    finally {
        cleanup(cwd);
    }
...
test('--signal-name without value throws clear error', async () => {
    ...
    try {
        await setupModel('proc', cwd);
        await assert.rejects(() => append(['signal-intermediate-throw-event', 'Throw', '--signal-name'], cwd), /--signal-name requires a value/);
    }
    finally {
        cleanup(cwd);
    }
...
test('--message-name without value throws clear error', async () => {
    ...
    try {
        await setupModel('proc', cwd);
        await assert.rejects(() => append(['message-intermediate-catch-event', 'Wait', '--message-name'], cwd), /--message-name requires a value/);
    }
    finally {
        cleanup(cwd);
    }
