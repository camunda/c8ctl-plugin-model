...
/* ------------------------------------------------------------------ */
async function lintBpmn(bpmnPath) {
...
test('best-practice: straight-through process with service tasks', async () => {
    ...
    try {
        ...
        const captured = [];
        const logger = { ...silentLogger(), json(d) { captured.push(d); } };
        await cursorStatus([], cwd, logger);
        assert.equal(captured[0]['cursor'], 'EndEvent_1');
        // -- verify status command output --
        const statusCaptured = [];
        const statusLogger = { ...silentLogger(), json(d) { statusCaptured.push(d); } };
...
test('best-practice: sub-process with boundary timer escalation', async () => {
    ...
    try {
        ...
        const children = subProc?.['children'];
...
test('best-practice: event sub-process for global error handling', async () => {
    ...
    try {
        ...
        const proc = s['process'];
        const elements = proc['elements'];
        const evtSubProc = elements.find((e) => e['id'] === 'Activity_2' && e['triggeredByEvent'] === true);
        assert.ok(evtSubProc, 'event sub-process exists with triggeredByEvent');
        const evtChildren = evtSubProc?.['children'];
        const errorStart = evtChildren.find((c) => c['eventDefinition']?.['type'] === 'error');
...
test('best-practice: parallel multi-instance service task', async () => {
    ...
    try {
        ...
        const loopChar = mi?.['loopCharacteristics'];
...
test('best-practice: text annotations on process elements', async () => {
    ...
    try {
        ...
        const s = await getStatus(cwd);
        const proc = s['process'];
        const artifacts = proc['artifacts'];
