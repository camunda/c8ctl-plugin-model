 ...
 */
export function extractIdFlag(args) {
    const idx = args.indexOf('--id');
    if (idx === -1)
        return { id: undefined, remaining: args };
 ...
 */
export function applyZeebeFlags(moddle, el, rawArgs, definitions, logger) {
    const { flags } = parseArgs(rawArgs);
    for (const [flagName, { prop, repeated, splitChar }] of Object.entries(FLAG_TO_PROP)) {
        if (COMMAND_ONLY_FLAGS.has(flagName))
            continue;
        if (repeated) {
