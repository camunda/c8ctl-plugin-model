...
export async function selectFile(args, cwd, logger) {
    const name = args[0];
    if (!name)
        throw new Error('Usage: c8ctl model select-file <path>');
    ...
    if (!cursor) {
        const first = (process.flowElements ?? []).find((e) => e.$type !== 'bpmn:SequenceFlow');
        if (!first)
            throw new Error(`No elements found in ${filePath}`);
        cursor = first.id;
        if (existingCursor)
            logger?.warn(`Cursor '${existingCursor}' not found in new file — reset to ${cursor}`);
    }
