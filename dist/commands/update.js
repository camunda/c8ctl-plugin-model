...
function looksLikeProperty(s) {
    if (s.startsWith('--'))
        return true;
    if (/^(?:name|id|isInterrupting|documentation|signalRef|messageRef)$/.test(s))
        return true;
    if (s.startsWith('zeebe:') || s.startsWith('timer.') ||
        s.startsWith('ad-hoc.') || s.startsWith('multi-instance.'))
        return true;
    return false;
}
export async function update(args, cwd, logger) {
    ...
    if (!el)
        throw new Error(`Element '${resolvedId}' not found`);
    ...
    if (!prop) {
        if (Object.keys(flags).length === 0) {
            throw new Error('Usage: c8ctl model update [elementId] <property> <value...>\n' +
